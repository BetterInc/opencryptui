...
void MainWindow::showSecurityTips(const QString &context)
{
    ...
    if (context == "file") {
        tips = "File Encryption - Security Tips:\n\n"
               "- Default to an AEAD cipher (AES-256-GCM or ChaCha20-Poly1305): they\n"
               "  encrypt AND authenticate, so any tampering is detected.\n"
               "- For the highest-value files, pick a cipher CASCADE - the file is\n"
               "  only broken if every cipher in the chain is.\n"
               "- AEAD files are written in the deniable v4 format: no magic bytes,\n"
               "  indistinguishable from random data on disk.\n"
               "- Add a keyfile (on a separate device) alongside your password for\n"
               "  critical data - both are then required.\n"
               "- Output files are created private (0600) automatically; you don't\n"
               "  need to fix permissions yourself.\n"
               "- Keep an independent encrypted backup - encryption is not backup.";
    }
    else if (context == "folder") {
        tips = "Folder Encryption - Security Tips:\n\n"
               "- Folders are tar-archived then encrypted as one AEAD file.\n"
               "- For something you'll open often, consider an encrypted CONTAINER\n"
               "  instead (File > Create Encrypted Container) - it can hold a hidden\n"
               "  volume and be mounted as a live drive.\n"
               "- Use a different password than you use elsewhere.\n"
               "- Test decryption after creating it, before deleting the original.\n"
               "- Store the encrypted archive somewhere only you can write to.";
    }
    else if (context == "disk") {
        tips = "Disk / USB Encryption - Security Tips:\n\n"
               "- Cross-platform, no admin: put an encrypted CONTAINER file on the\n"
               "  USB (File > Create Encrypted Container). It can include a hidden\n"
               "  volume and be mounted on demand.\n"
               "- Whole-device encryption (erasing the entire USB) is Linux-only for\n"
               "  now and DESTROYS existing data - back up first.\n"
               "- Decryption writes plaintext to disk; secure-wipe it when done.\n"
               "- For a hidden volume, fill the outer (decoy) volume with believable\n"
               "  data - an empty decoy is itself suspicious.\n"
               "- Whole-system/boot encryption is best handled by your OS\n"
               "  (LUKS / BitLocker / FileVault), not this tool.";
    }
    else {
        tips = "General Security Tips:\n\n"
               "- Use a long passphrase (16+ chars / several random words).\n"
               "- Argon2id is the default KDF (memory-hard, ~1 GiB) - keep it unless\n"
               "  you have a specific reason; it's far stronger than PBKDF2.\n"
               "- Split a password into key SHARES (k-of-n) so no single seized\n"
               "  share - or coerced person - can unlock anything.\n"
               "- Store keyfiles/shares on separate physical devices.\n"
               "- Never send a password over email/chat; beware shoulder-surfing.\n"
               "- Biggest caveat: this tool is NOT independently audited - for\n"
               "  truly high-stakes data, weigh that against any feature.";
    }
